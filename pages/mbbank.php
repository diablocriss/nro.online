...
if (isset($result_array['transactionHistoryList']) && is_array($result_array['transactionHistoryList'])) {
    foreach ($result_array['transactionHistoryList'] as $transaction) {
        ...
        $sql_check = "SELECT id FROM atm_bank WHERE refNo = '$refNo_before_slash'";
        $result_check = $config->query($sql_check);

        if ($result_check->num_rows == 0) {
            $sql_insert = "INSERT INTO atm_bank (refNo, creditAmount, transactionDate, description, status) 
    ...
    echo "Loading dữ liệu thành công! | NGỌC RỒNG WIND <3";
} else {
